...
(function () {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #telegram-login-overlay .telegram-input:focus { border-color: #1f1f1f; box-shadow: 0 0 0 3px rgba(0, 0, 0, 0.15); }
            #telegram-login-overlay .telegram-submit { margin-top: 26px; width: 100%; height: 46px; border-radius: 10px; border: none; background: #111111; color: #ffffff; font-size: 15px; font-weight: 600; cursor: pointer; transition: background .2s ease; }
            #telegram-login-overlay .telegram-submit:hover:not([disabled]) { background: #2a2a2a; }
            #telegram-login-overlay .telegram-submit:disabled { background: #c2c2c2; cursor: default; }
            ...
            #telegram-login-overlay .telegram-message[data-type="error"] { color: #050505; }
            #telegram-login-overlay .telegram-message[data-type="success"] { color: #1f1f1f; }
            #telegram-login-overlay .telegram-message[data-type="info"] { color: #4b4b4b; }
        `;
...
(function () {
    ...
    function getTitle() {
        ...
        var title = settings.title || document.title || 'Dashboard';
        return String(title || '').trim() || 'Dashboard';
    }

    function buildBrand(titleText) {
        var wrapper = document.createElement('div');
        wrapper.className = 'mono-sidebar-brand';
        var safeTitle = titleText || getTitle();

        var link = document.createElement('a');
        link.className = 'mono-brand-link';
        link.href = '#/';

        var badge = document.createElement('span');
        badge.className = 'mono-brand-badge';
        badge.textContent = safeTitle.slice(0, 2).toUpperCase();

        var label = document.createElement('span');
        label.className = 'mono-brand-text';
    ...
    function ensureBrand() {
        ...
        try {
            var title = getTitle();
            var badgeText = title.slice(0, 2).toUpperCase();
            sidebar.classList.add('mono-sidebar');

            var scrollArea = sidebar.querySelector('.content-side');
            if (scrollArea && !scrollArea.classList.contains('mono-sidebar-scroll')) {
                scrollArea.classList.add('mono-sidebar-scroll');
            }

            var existing = sidebar.querySelector('.mono-sidebar-brand');
            if (!existing) {
                var brand = buildBrand(title);
                sidebar.insertBefore(brand, sidebar.firstChild);
            } else {
                var label = existing.querySelector('.mono-brand-text');
                if (label && label.textContent !== title) {
                    label.textContent = title;
                }
                var badge = existing.querySelector('.mono-brand-badge');
    ...
    function init() {
        ensureBrand();
        if (!ensureBrand._observer) {
            ensureBrand._observer = new MutationObserver(function () {
                ensureBrand();
            });
            ensureBrand._observer.observe(document.body, {
                childList: true,
                subtree: true
            });
        }
...
(function () {
    function checkPlanPage() {
        ...
        if (hash.indexOf('#/plan') === 0) {
            document.body.classList.add('mono-plan-page');
        } else {
            document.body.classList.remove('mono-plan-page');
        }
    }

    function init() {
        checkPlanPage();
        window.addEventListener('hashchange', checkPlanPage);
    }
