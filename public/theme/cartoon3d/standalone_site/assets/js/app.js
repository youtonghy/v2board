document.addEventListener('alpine:init', () => {
    Alpine.data('app', () => ({
        ...
        applyThemeFromSettings() {
            const color = window.settings?.theme?.color;
            ...
            const palette = {
                default: {
                    ...
                    bg: '#e6f0f8',
                    card: '#eef4fa',
                    dropdownHover: 'rgba(79, 156, 255, 0.12)'
                },
                purple: {
                    primary: '#6c5ce7',
                    secondary: '#a29bfe',
                    bg: '#efeefe',
                    card: '#f4f2ff',
                    dropdownHover: 'rgba(108, 92, 231, 0.12)'
                },
                orange: {
                    primary: '#f97316',
                    secondary: '#fdba74',
            ...
            const selected = (typeof color === 'string' && palette[color]) ? palette[color] : palette.default;
            root.style.setProperty('--primary-color', selected.primary);
            root.style.setProperty('--secondary-color', selected.secondary);
            root.style.setProperty('--bg-color', selected.bg);
            root.style.setProperty('--card-bg', selected.card);
            root.style.setProperty('--dropdown-hover', selected.dropdownHover);

            const bg = window.settings?.background_url;
            if (typeof bg === 'string' && bg.trim() !== '') {
                document.body.style.backgroundImage = `url("${bg.trim()}")`;
        ...
        getApiBase() {
            const raw = window.settings?.api_base;
        ...
        getBrandTitle() {
            const configured = window.settings?.title;
            if (typeof configured === 'string' && configured.trim() !== '') return configured.trim();

            const appUrl = this.siteConfig?.app_url;
        ...
        applyBranding() {
            const title = this.getBrandTitle();
            document.title = title;

            const brand = document.getElementById('fantastic-brand');
            if (!brand) return;

            const logoUrl = (typeof window.settings?.logo === 'string' && window.settings.logo.trim() !== '')
                ? window.settings.logo.trim()
                : (typeof this.siteConfig?.logo === 'string' ? this.siteConfig.logo.trim() : '');
        ...
        applyCustomHtml() {
            const el = document.getElementById('fantastic-custom-html');
            if (!el) return;
            const html = window.settings?.custom_html;
        ...
        async fetchSiteConfig() {
            try {
                ...
                if (data.data) {
                    ...
                    if (typeof data.data.telegram_login_enable !== 'undefined') {
                        this.telegram_login_enable = Number(data.data.telegram_login_enable) || 0;
                    }
                    if (typeof data.data.sso_login_enable !== 'undefined') {
                        this.sso_login_enable = Number(data.data.sso_login_enable) || 0;
                    }
