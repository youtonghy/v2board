...
// SkyBlue theme tweaks: add body class and ripple animation for buttons
(function () {
    function addThemeClass() {
        if (document && document.documentElement) {
            document.documentElement.classList.add('skyblue-theme');
        }
    ...
    function attachRipple(node) {
        if (!node || node.dataset.skyRipple === '1') return;
        node.dataset.skyRipple = '1';
        node.classList.add('sky-ripple');
        node.addEventListener('click', function (event) {
            if (node.disabled) return;
            var existing = node.querySelector('.sky-ripple-wave');
            ...
            ripple.className = 'sky-ripple-wave';
    ...
    function scanButtons() {
        var nodes = document.querySelectorAll('button, .ant-btn, .btn, .am-button');
