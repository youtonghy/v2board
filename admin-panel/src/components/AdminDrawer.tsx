    ...
    <Drawer>
      ...
      >
        <Drawer.Content placement="right" className={drawerWidthClassNames[size]}>
          <Drawer.Dialog className="flex h-full flex-col border-l border-white/60 bg-white/95 shadow-[0_24px_80px_rgba(15,23,42,0.18)] backdrop-blur-xl">
            ...
            <Drawer.Header className="shrink-0 border-b border-slate-100 px-6 py-5">
              <Drawer.Heading className="pr-10 text-lg font-semibold tracking-[-0.02em] text-slate-950">
                {title}
              </Drawer.Heading>
            </Drawer.Header>
            <Drawer.Body className="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-5">
              {children}
            </Drawer.Body>
            {footer ? (
              <Drawer.Footer className="shrink-0 flex flex-wrap justify-end gap-3 border-t border-slate-100 px-6 py-4">
                {footer}
              </Drawer.Footer>
            ) : null}
          </Drawer.Dialog>
